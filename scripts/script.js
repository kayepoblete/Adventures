...
// 2. Selecting a category, find matching activites
// 3. Output to table


window.onload = () => {
    createNewDropdown(categories, "#categoriesList");
    document.querySelector("#activitiesTable").style.display = "none";
    const categoriesList = document.querySelector("#categoriesList");
    categoriesList.onchange = loadActivitiesTable;
};
...
const createNewDropdown = (_myArrayList, _nameOfDropdown) => {
    ...
    //     newDropdown.appenChild(theOption);
    // });
    let numElements = _myArrayList.length;
...
// Create table
const loadActivitiesTable = () => {
    const table = document.querySelector("#activitiesTable");
    const index = document.querySelector("#categoriesList").selectedIndex;
    const selectedCategoryText = document.querySelector("#categoriesList")[index].text;
    let numActivities = activities.length;
    // Hide table when "Select one..." is chosen
    if(selectedCategoryText === "Select one..."){
        document.querySelector("#activitiesTable").style.display = "none";
    }
    else{
        document.querySelector("#activitiesTable").style.display = "block";
    }
    // Populate table
    for(let i = 0; i < numActivities; i++){
        if(activities[i].category === selectedCategoryText){
            ...
            cell2.innerHTML = activities[i].id;
            let cell3 = row.insertCell(2);
            cell3.innerHTML = activities[i].description;
            let cell4 = row.insertCell(3);
            cell4.innerHTML = activities[i].location;
            let cell5 = row.insertCell(4);
            cell5.innerHTML = activities[i].price;
        }
    }
};
